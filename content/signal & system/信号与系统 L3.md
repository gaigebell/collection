...
$$
e^{st} \longrightarrow \fbox{$h(t)$} \longrightarrow y(t)
$$
$$
z^n \longrightarrow \fbox{$h[n]$} \longrightarrow y[n] 
$$

$$ 
y(t) = \int_{-\infty}^{\infty} e^{s(t-\tau)} h(\tau)\;{\rm d} \tau \;\;(卷积) = e^{st} \fbox{\color{red} $\int_{-\infty}^{\infty} h(\tau) s^{-s\tau}\;{\rm d} \tau$ } = {\color{red} H(s)} e^{st}
$$

- 特征函数：系统对某一信号的响应是该信号乘以一个函数，称信号为特征函数 $e^{st}, z^n$
- 特征值：对应的常数 $H(s), H(z)$
...
> 基波频率 $\omega_0 = 2\pi / T$
> 
> $a_0 = \dfrac{1}{T} \int_{-T_1}^{T_1}\;{\rm d} t = \dfrac{2T_1}{T}$
> 
> $a_k = \dfrac{1}{T} \int_{-T_1}^{T_1}1\cdot e^{-jk\omega_0 t}\;{\rm d} t  = -\dfrac{1}{jk\omega_0 T} e^{-jk\omega_0 t}\big|_{-T_1}^{T_1} = \dfrac{\sin(k\omega_0 T_1)}{k\pi}$
> 
> 傅里叶级数系数反映了信号中每个基本信号成分的贡献，也就是说频谱反映了信号的（由基本信号组合）组成成分
> 
> 类似的频谱（函数？）：
> - 抽样信号 ${\rm Sa}(x) = \dfrac{\sin x}{x}$
> - ${\rm sinc}(x) = \dfrac{\sin \pi x}{\pi x}$


> [!important]- 傅里叶级数是对周期性信号的最佳近似
> 误差 $e_N(t) = x(t)-x_N(t)$
> ...
> 
...
> [!danger] 另一组充分条件
> 平方可积条件
> 如果 $\int_T|x(t)|^2\;{\rm d} t < \infty$ 则 $a_k$ 必存在
...
如果有目标信号有间断点，近似信号在间断点附近不可避免会出现振荡和超量，随着项数增多，振荡频率变高，并向间断点处压缩，幅度却不减小：使振荡==占有== 的能量减少
